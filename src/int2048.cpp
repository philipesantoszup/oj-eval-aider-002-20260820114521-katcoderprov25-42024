...
int2048 int2048::absDiv(const int2048 &other) const {
    ...
    multiples[0] = {0};
    for (int d = 1; d <= 9; ++d) {
        std::vector<int> &prev = multiples[d - 1];
        std::vector<int> &cur = multiples[d];
        int carry = 0;
        cur.resize(other.digits.size() + 1, 0);
        for (size_t k = 0; k < other.digits.size() || carry; ++k) {
            long long v = carry + (long long)(k < other.digits.size() ? other.digits[k] : 0) * d;
            // Actually we want other*d, not (other*(d-1)) + other. Let me just compute directly.
        }
        // Recompute directly:
        cur.clear();
        int c2 = 0;
